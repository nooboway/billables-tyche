...
export default function SignupScreen() {
  ...
  async function onSubmit(data: FormData) {
    try {
      ...
      setError("email", { message: e.response?.data?.error ?? "Registration failed" });
    }
  }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          {(["businessName", "email", "password"] as const).map((name) => (
            <Controller
              key={name}
              control={control}
              name={name}
              render={({ field: { value, onChange, onBlur }, fieldState: { error } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>
                    {name === "businessName" ? "Business Name" : name === "email" ? "Email" : "Password"}
                  </Text>
                  <TextInput
                    ...
                    onBlur={onBlur}
                    placeholder={name === "businessName" ? "Your Business Ltd." : name === "email" ? "you@example.com" : "••••••••"}
        ...
        <TouchableOpacity onPress={() => nav.navigate("Login")} style={styles.switchLink}>
...
const styles = StyleSheet.create({
  ...
  wordmark: { color: "#ffffff", fontSize: 36, fontWeight: "900", textAlign: "center", marginBottom: 8 },
  ...
  input: {
    ...
    borderRadius: 8, paddingHorizontal: 14, paddingVertical: 12, color: "#ffffff", fontSize: 15,
  },
